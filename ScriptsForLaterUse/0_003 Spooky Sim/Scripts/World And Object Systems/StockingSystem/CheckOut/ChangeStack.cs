...
public class ChangeStack : MonoBehaviour
{
    ...
    public GameObject PushClone(GameObject prefab = null, Transform prefabParent = null)
    {
        var p = prefab ? prefab : defaultPrefab;
        var originalParent = prefabParent != null ? prefabParent : anchor;
        if (!p)
        {
            Debug.LogError($"{name}: PushClone called without a prefab.");
            return null;
        }
        var inst = Instantiate(p, prefabParent.position, prefabParent.rotation);
        inst.transform.SetParent(prefabParent, worldPositionStays: true);
        Push(inst);
        return inst;
    }
 ...
 /*   /// <summary>Remove a specific item from the stack (wherever it is). Reflows remaining items.</summary>
    ...
    public void PopAtStackOfGameobject(GameObject item)
    {
        if (!item) return;
        Transform popItem = item.transform.parent.GetChild(item.transform.parent.childCount - 1);


        int idx = _items.IndexOf(popItem);
        if (idx < 0) return;

        _items.RemoveAt(idx);
        RestoreAndUnparent(popItem);

        // Reflow positions for items after the removed index
        //for (int i = item.transform.parent.childCount -idx; i < item.transform.parent.childCount; i++)
            //PlaceAtIndex(i, _items[i]);

        Destroy(popItem.gameObject);
    }
    ...
    private void PlaceAtIndex(int index, Transform t)
    {
       ...
       /* // base position/orientation in anchor's local axes
        ...
        // base position/orientation in anchor's local axes
        Vector3 pos = t.parent.position
                     // + t.parent.right * (gridX * columnGridSpacing.x)
                      //+ t.parent.forward * (gridZ * columnGridSpacing.y)
                      + t.parent.up * (t.parent.childCount * itemHeight);
