...
public class Carryable : MonoBehaviour
{
    ...
    public bool isOnCheckoutCounter = false;
    ...
    void LateUpdate()
    {
        if(!isOnCheckoutCounter) return;
        if (bounds == null)
        {
            bounds = itemReadyForCheckout.checkOutCounter.placementArea.GetComponent<BoxCollider>();
        }
    ...
    public void ScanMe()
    {
        ...
        Debug.Log("ScanMe called from Carryable.");
        ActivateColliders(true);
        isOnCheckoutCounter = false;
    }
    public void AttachToHolder(Transform holder)
    {
        Debug.Log("AttachToHolder called from Carryable.");
        if (!_rb) return;
    ...
    private void ActivateColliders(bool activate)
    {
        ...
        foreach (var c in _cols) c.enabled = activate;
    }
}
