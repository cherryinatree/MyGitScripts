...
public class CustomerShopping : MonoBehaviour
{
    ...
    private void GrabItemFromShelf(Carryable item)
    {
        ...
        if (item.GetFixtureParent() == null) return;
        if (item.GetFixtureParent().FindShelfSlot(item.gameObject) == null) return;
        item.GetFixtureParent().FindShelfSlot(item.gameObject).ClearSlot();
    }
    ...
    public void PutItemInBag()
    {
        ...
        if(shoppingList[0] == null) { shoppingList.RemoveAt(0); return; }

        if (Vector3.Distance((shoppingList[0]).transform.position, transform.position) < 3f)
        {
