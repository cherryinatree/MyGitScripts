...
[CustomPropertyDrawer(typeof(CombatTranistion))]
public class Editor_CombatTransitionDraw : PropertyDrawer
{
    ...
    {
        CombatDecision thisDecision = prop.objectReferenceValue as CombatDecision;
        CombatDecision[] decisions = (prop.serializedObject.targetObject as CombatStateMachine).GetAttachedDecisions();
        int selected = 0;
        int i = 1;
        string[] options = new string[decisions.Length + 1];
        options[0] = "None";
        foreach (CombatDecision decision in decisions)
        {
