...
namespace MoreMountains.CorgiEngine
{
    ...
    /// </summary>
    [AddComponentMenu("Corgi Engine/Character/Abilities/Character Run Better")]
    public class CharacterRunBetter : CharacterAbility
    {
        ...
        {
            //playerStamina.Stamina += StaminaRegenRate * Time.deltaTime;
            //Stamina = Mathf.Clamp(Stamina, 0, MaxStamina);
            if (_movement.CurrentState == CharacterStates.MovementStates.Running)
            {

                playerStamina.Stamina -= Time.deltaTime * StaminaDepletionRate;
            }
            if(_movement.CurrentState == CharacterStates.MovementStates.Jumping ||
                _movement.CurrentState == CharacterStates.MovementStates.Jumping || _movement.CurrentState == CharacterStates.MovementStates.WallJumping)
            {

                playerStamina.Stamina -= Time.deltaTime * StaminaDepletionRate/4;
            }


            if (playerStamina.Stamina < StaminaCutThreshold)
            {
                RunStop();
                return;
            }
            if (playerStamina.Stamina < StaminaActivateThreshold)
            {
            ...
            if (_inputManager.TimeControlButton.State.CurrentState == MMInput.ButtonStates.ButtonDown || _inputManager.TimeControlButton.State.CurrentState == MMInput.ButtonStates.ButtonPressed)
            {
                RunStart();
            }
            if (_inputManager.TimeControlButton.State.CurrentState == MMInput.ButtonStates.ButtonUp)
            ...
            if (AutoRun)
            {
                ...
                {
                    _inputManager.TimeControlButton.State.ChangeState(MMInput.ButtonStates.ButtonPressed);
                }
                else
                {
                    _inputManager.TimeControlButton.State.ChangeState(MMInput.ButtonStates.ButtonUp);
                    RunStop();
                }
            }
