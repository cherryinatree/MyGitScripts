...
public class PlayerMovement 
{
    ...
    public PlayerMovement(Transform playerTransform, PlayerInput playerInput, PlayerData Data, 
        ClassList classList, LayerMask groundLayer, Vector2 groundCheckSize, Vector2 wallCheckSize,
        Transform groundCheckPoint, Transform frontWallCheckPoint, Transform backWallCheckPoint)
    {
        ...
        _frontWallCheckPoint = player.transform.Find("FrontWallCheck");
        _backWallCheckPoint = player.transform.Find("BackWallCheck");
        _groundCheckPoint = player.transform.Find("GroundCheck");
    ...
    public void UpdateMovement()
    {
        ...
        if (!IsJumping)
        {
            ...
            //Right Wall Check
            if (((Physics.OverlapBox(_frontWallCheckPoint.position, _wallCheckSize, Quaternion.identity, _groundLayer).Length > 0 && IsFacingRight)
                    || (Physics.OverlapBox(_backWallCheckPoint.position, _wallCheckSize, Quaternion.identity, _groundLayer).Length > 0 && !IsFacingRight)) && !IsWallJumping)
        ...
        //Apply gravity
        RB.AddForce(-Vector2.down * Data.gravityStrength * gravityScale, ForceMode.Acceleration);
        #endregion
    }
    ...
    private void Turn()
    {
       ...
       //float direction = playerTransform.rotation.y;
