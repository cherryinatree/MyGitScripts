...
public class VehicleControls : MonoBehaviour
{
    ...
    public void VC_Start(Rigidbody rBody)
    {
        ...
        rb = rBody;
        whellCols = rb.gameObject.transform.Find("WheelCollider").GetComponentsInChildren<WheelCollider>();

        whellMeshs = new Transform[rb.gameObject.transform.Find("WheelMesh").childCount];
        for (int i = 0; i < rb.gameObject.transform.Find("WheelMesh").childCount; i++)
        {
            whellMeshs[i] = rb.gameObject.transform.Find("WheelMesh").GetChild(i).GetComponent<Transform>();
        }

        gearRatios = new float[] { 4,4,3,2,4}; // Example gear ratios
    ...
    void simpleControl()
    {
        ...
        if (fixedRpm < maxRpm - 500)
        {
            if (gear == gearRatios.Length - 1)
            {
                ...
                gasPedal = breaknow * maxRpm;
                addBreakTorq(driveInput.y * breakPower);
            ...
            else
            {
                ...
                gasPedal = driveInput.y * maxRpm;
                addBreakTorq(breaknow * breakPower);
                addTorq(calculateCurrentTorq(gear, gasPedal) * reversingGear);
            }
        }
        else
    ...
    public float currentSpeed()
    {
        ...
        if (m_speedType == SpeedType.kmh)
            speed = rb.linearVelocity.magnitude * 3.6f; //is multiplied by a 3.6 kmh
        if (m_speedType == SpeedType.mph)
            speed = rb.linearVelocity.magnitude * 2.2f; //is multiplied by a 3.6 mph
