...
public class CarryAction : PlayerAction
{
    ...
    public void SetBusy(bool busy) => _busyAnimating = busy;
    public void ReplaceHeld(Carryable newHeld) => _held = newHeld;
    ...
    protected override void Subscribe(CorePlayer c)
    {
        ...
        c.OnInteractCanceled += OnButtonRaised;
    }
    ...
    private void OnInteractPressed()
    {
        ...
        if (!unpressed) return;
        // If holding a delivery box, E opens it (priority)
        if (_held != null && _held.TryGetComponent<DeliveryBox>(out var deliveryBox))
        {
            deliveryBox.TryOpen(this);
            return;
        }
        ...
        if (_held && _held.TryGetComponent<BoxContainer>(out var box))
        {
            if (TryRaycast(out var hit))
            {
                ...
                if (hit.collider.TryGetComponent<Carryable>(out var carry))
                {
                    Debug.Log("Trying to store box contents into carryable");
                    if (_held.GetComponent<BoxContainer>().TryStore(hit.collider.GetComponent<Carryable>()))
                    {
                        Debug.Log("Trying to store box contents into carryable");
                    }
                    return;
