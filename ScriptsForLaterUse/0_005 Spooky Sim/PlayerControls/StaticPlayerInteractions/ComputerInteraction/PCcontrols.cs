...
public class PControls : PlayerAction
{
    ...
    [Header("UI Raycast (Topmost)")]
    [SerializeField] private GraphicRaycaster graphicRaycaster; // usually on the Canvas
    [SerializeField] private EventSystem eventSystem;           // scene EventSystem
    ...
    private ComputerInteractable switchControls;
    ...
    [Header("Input")]
    [Tooltip("Assign an Input Action (e.g. Player/Interact) bound to <Keyboard>/e")]
    [SerializeField] private InputActionReference flopAction;
    ...
    protected override void Awake()
    {
        ...
        if (graphicRaycaster == null && screenArea != null)
            graphicRaycaster = screenArea.GetComponentInParent<GraphicRaycaster>();
        ...
        {
            switchControls = FindAnyObjectByType<ComputerInteractable>();
        }
    ...
    private void Update()
    {
        ...
        if (Time.time < _nextAllowedFlopTime)
            return;
        // NEW INPUT SYSTEM: Press E (or whatever your action is bound to)
        if (flopAction != null && flopAction.action.WasPressedThisFrame())
        {
            switchControls.SwitchToPlayer(); // <-- your flop call
    ...
    private PCClickable GetTopmostUIUnderHotspot()
    {
        if (graphicRaycaster == null || _ped == null) return null;


        _ped.position = HotspotScreenPoint();
        _uiHits.Clear();
        graphicRaycaster.Raycast(_ped, _uiHits);
        //testShphere.transform.position = _ped.position;

        // Already sorted: topmost first
        for (int i = 0; i < _uiHits.Count; i++)
        {
            var c = _uiHits[i].gameObject.GetComponentInParent<PCClickable>();
    ...
    {
        // Prefer UI raycast (handles stacked windows correctly)
       // var uiTop = GetTopmostUIUnderHotspot();
        //if (uiTop != null) return uiTop;
       // Debug.Log("No uiTop");
        ...
        if (hitTest == HitTestMode.UIRectContains || hitTest == HitTestMode.Auto)
        {
            ...
            {
                if (!c.TryRectContains(sp, uiCamera)) continue;

                long score = GetUiSortScore(c);
                if (score > bestScore)
                {
                    bestScore = score;
                    best = c;
...
private long GetUiSortScore(PCClickable c)
{
    // Canvas priority (layer + order)
    var canvas = c.GetComponentInParent<Canvas>();
    int layerValue = canvas ? SortingLayer.GetLayerValueFromID(canvas.sortingLayerID) : 0;
    int sortingOrder = canvas ? canvas.sortingOrder : 0;

    // “Topmost” within the canvas: biggest Graphic.depth wins
    int maxDepth = GetMaxGraphicDepth(c.transform);

    // Composite score (bigger = more on top)
    // Pack into a long to compare quickly.
    // (layerValue) then (sortingOrder) then (depth)
    return ((long)layerValue << 40) | ((long)(sortingOrder & 0xFFFF) << 16) | (ushort)(maxDepth & 0xFFFF);
}

private int GetMaxGraphicDepth(Transform root)
{
    int best = int.MinValue;

    // Include children because often the clickable is on a parent, while the Image is on a child
    var graphics = root.GetComponentsInChildren<Graphic>(includeInactive: false);
    for (int i = 0; i < graphics.Length; i++)
        best = Mathf.Max(best, graphics[i].depth);

    // Fallback if there are no Graphics (rare for UI): use sibling index
    if (best == int.MinValue) best = root.GetSiblingIndex();
