...
public class CombatStateMachine : MonoBehaviour
{
    ...
    private static string N(string s) => string.IsNullOrEmpty(s) ? "" : s.Trim();
    ...
    {
        newStateName = N(newStateName);

        //Debug.Log("Transition to: " + newStateName);
        ...
        if (!string.Equals(N(CurrentState.StateName), newStateName, System.StringComparison.OrdinalIgnoreCase))
        {
    ...
    protected CombatState FindState(string stateName)
    {
        ...
        foreach (CombatState state in States)
        {
            if (string.Equals(N(state.StateName), stateName, System.StringComparison.OrdinalIgnoreCase))
                return state;
        }

        if (stateName != "")
            Debug.LogError($"No state named '{stateName}' exists on {gameObject.name}. " +
                           $"Available: {string.Join(", ", States.ConvertAll(s => $"'{N(s.StateName)}'"))}");
